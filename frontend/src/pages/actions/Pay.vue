<template>
  <BRow>
    <BForm @submit="onSubmit">
      ...
      <BFormGroup label="Amount:" label-for="input-5">
        <BFormInput type="number" id="input-5" v-model="form.value" placeholder="Enter amount" required/>
        <BAlert v-model="v$.value.$error" variant="danger" dismissible>
          <p v-for="error in v$.value.$errors">
            {{ error.$message }}
          </p>
        </BAlert>
      </BFormGroup>
...
import {pay} from '@/services/actions';
...
const onSubmit = async (event: Event) => {
  ...
  try {
    ...
    if (data !== undefined && data.status == 200) {
      emits("onCloseModal", {modal: 'pay', document: document, phone: phone});
    }
  } catch (error) {
