  ...
  <BRow>
    <BForm @submit="onSubmit">
      ...
      <BFormGroup label="Amount:" label-for="input-5">
        <BFormInput type="number" id="input-5" v-model="form.balance" placeholder="Enter amount" required/>
        <BAlert v-model="v$.balance.$error" variant="danger" dismissible>
          <p v-for="error in v$.balance.$errors">
            {{ error.$message }}
          </p>
        </BAlert>
      </BFormGroup>
...
const onSubmit = async (event: Event) => {
  ...
  try {
    ...
    let data = await deposit(form.value);
    ...
    if (data !== undefined && data.status == 200) {
      alert(data.data)
      emits("onCloseModal", {modal: 'deposit', document: document, phone: phone});
    }
  } catch (error) {
